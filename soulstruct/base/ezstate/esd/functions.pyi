...
from soulstruct.base.events.emevd.enums import ComparisonType, ChangeType, ItemType, PlayerStats
from soulstruct.game_types.internal_types import ESDType
from soulstruct.game_types.param_types import DialogueParam
...
# command talk[1][1]
def TalkToPlayer(conversation: DialogueParam, unk1: int, unk2: int):
    """ conversation: id of DialogueParam\n
        unk1: always -1\n
        unk2: always -1\n
        will attempt to also play consecutive DialogueParams """
    ...
...
# command talk[1][17]
def OpenGenericDialog(unk1: int, text_id: EventText, unk2: int, unk3: int, display_distance: int): 
    """ TODO: enums\n
    unk1:
        1: ?\n
        3: yes/no with number input\n
        7: only yes\n
        8: yes/no """
    ...
...
# command talk[1][55]
def SetAquittalCostMessageTag(factor1: int, factor2: int):
    """ sets the number associated with the <?evntAcquittalPrice?> tag to be
    equal to the player's soul level multiplied by both factors """
    ...

# command talk[1][56]
def SubtractAcquittalCostFromPlayerSouls(factor1: int, factor2: int): 
    """ the amount of souls subtracted is the player's soul level multiplied by both arguments\n
    if this amount is greater than the current soul count, the soul count goes to zero """
    ...
...
# test talk[16]
def IsEquipmentIDObtained(item_type: ItemType, item_id: int):
    """ returns true if an item of given type is obtained\n
    TODO: does this include bottomless box? """
    ...

# test talk[17]
def IsEquipmentIDEquipped(item_type: ItemType, item_id: int):
    """ returns true if an item of given type is equipped """
    ...
...
def GetGenericDialogButtonResult() -> int:
    """ TODO: more testing with different arguments to OpenGenericDialog\n
    based on preliminary testing, returning:\n
        0: pressing B to back out of the menu\n
        1: selecting YES\n
        2: selecting NO """
    ...
...
# test talk[47]
def ComparePlayerInventoryNumber(item_type: ItemType, item_id: int, comparison_type: ComparisonType, value: int, count_bottomless_box: bool): ...
